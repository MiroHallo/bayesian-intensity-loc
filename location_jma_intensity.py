#!/usr/bin/env python3
# =============================================================================
# EARTHQUAKE EPICENTER LOCATION FROM JMA INSTRUMENTAL SEISMIC INTENSITY
#
...
# Forward problem (vectorized over stations, JAX)
@jax.jit
def forward_jma_intensity(
        mw: float, r: jnp.ndarray, h_top: float, vs30: jnp.ndarray
        ) -> tuple[jnp.ndarray, float]:
    """
    JMA seismic intensity prediction (Morikawa and Fujiwara, 2013).
...
def eval_log_L(
    ...
    Args:
        ...
        int_obs (jnp.ndarray): Array of observed JMA instrumental intensities.
    ...
    r = jnp.sqrt((x_src - x_st)**2 + (y_src - y_st)**2 + h_top**2)
...
def check_memory_requirements(gx: jnp.ndarray, gy: jnp.ndarray, gz:
    ...
    # Check RAM
    if res_size_gb > available_ram_gb * 0.95:
        raise MemoryError(
    ...
    if jax_slice_gb > 2.0:
        print("ADVISORY: Large slice, consider reducing X-Y grid resolution")
...
def main() -> None:
    """
    Main function for the location from JMA instrumental seismic intensity.

    - Read input JMA seismic intensity and Vs30 from the INPUT.input_file.
    ...
    # Get JMA instrumental seismic intensity
    ...
    # Get Vs30 from JSHIS if Vs30 is missing (value <= 0)
    if n_bad > 0:
        ...
        # Save the updated table with new Vs30 values
        output_path = input_path.with_name(f"UPDATED_{input_path.name}")
        try:
            out_df = pd.DataFrame(inp_data)
            out_df['vs30'] = data['vs30']
    ...
    x_st = jnp.array(data['x_km'].values)
    y_st = jnp.array(data['y_km'].values)
    vs30_st = jnp.array(data['vs30'].values)
    int_obs = jnp.array(data['int'].values)
    sig_obs = jnp.array(data['sig'].values)
    ...
    r_bes = np.sqrt((loc_res[0] - x_st)**2 + (loc_res[1] - y_st)**2 + h_top**2)
